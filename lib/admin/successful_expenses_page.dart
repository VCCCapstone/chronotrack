...
class SuccessfulExpensesPage extends StatefulWidget {
  const SuccessfulExpensesPage({super.key});

  @override
  State<SuccessfulExpensesPage> createState() => _SuccessfulExpensesPageState();
}

class _SuccessfulExpensesPageState extends State<SuccessfulExpensesPage> {
  ...
  Future<void> fetchExpenses() async {
    ...
    final url = Uri.parse(
      'https://b93r46mokk.execute-api.ca-central-1.amazonaws.com/prod/expense/list-images?folder=successful',
    );
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint('Failed to fetch successful expenses: ${response.body}');
    }
  }
  ...
  Future<void> _deleteFile(Map<String, dynamic> expense) async {
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        'filename': filename,
        'folder': 'successful', // Only send the S3 file name
      }),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Successful Expense Receipts'),
            ...
            _loading
                ...
                : Expanded(
                    child: _filtered.isEmpty
                        ? const Center(
                            child: Text('No successful receipts found.'),
                          )
                        : ListView.builder(
